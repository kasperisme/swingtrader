...
export function RelationshipNetworkExplorer(props: RelationshipNetworkExplorerProps) {
  ...
  const showBootstrapSpinner = hideSeedControls && loading && nodes.length === 0 && !error;
  ...
  async function loadNeighborhood(resetSelection = true, seedOverride?: string) {
    ...
    if (resetSelection) {
      ...
      const seedNode =
        seed.length > 0
          ? result.data.nodes.find((n) => n.toUpperCase() === seed.toUpperCase()) ?? null
          : null;
