...
_TOOL_MAP_USER = {
    "get_user_positions": lambda user_id: get_user_positions(user_id),
    "get_user_alerts": lambda user_id: get_user_alerts(user_id),
    "get_user_screening_notes": lambda user_id: get_user_screening_notes(user_id),
    "search_news": lambda _uid, **kw: search_news(**kw),
}
...
def _call_tool(name: str, args: dict, user_id: str | None = None) -> Any:
    ...
    user_fn = _TOOL_MAP_USER.get(name)
    if user_fn:
        try:
            if name == "search_news":
                return user_fn(user_id, **args)
            else:
                return user_fn(user_id)
        except Exception as exc:
            return {"error": str(exc)}
