...
Examples (from code/analytics/, with .env APIKEY set):
...
def main() -> None:
    ...
    for symbol in ls_symbol:
        ...
        try:
            ...
            if args.save_charts:
                output_dir = f"{_ROOT}/output/screening/{now}/{symbol}"
    ...
    if args.excel:
        out_xlsx = str(_ROOT / "output" / "IBD_subset_trend_template.xlsx")
        os.makedirs(str(_ROOT / "output"), exist_ok=True)
