...
def update_scan_job_progress(job_id: int, message: str) -> None:
    client = get_supabase_client()
    _tbl(client, "user_scan_jobs").update({"progress_message": message}).eq(
        "id", job_id
    ).execute()
...
def persist_market_wide_scan(
    ...
    run_id = insert_scan_run(
        client, scan_date, source, market_json=market_json, user_id=user_id
    )
    append_scan_rows(
        client, run_id, scan_date, "trend_template", trend_template, user_id=user_id
    )
...
def save_article_tickers(
    ...
    _tbl(client, "news_article_tickers").delete().eq("article_id", article_id).eq(
        "source", source
    ).execute()
    if tickers:
...
def load_article_tickers(
    ...
    """
    q = (
        _tbl(client, "news_article_tickers")
        .select("ticker")
...
def patch_news_article_image_if_missing(
    ...
    if cur is not None and str(cur).strip():
        return
    _tbl(client, "news_articles").update({"image_url": url}).eq(
        "id", article_id
    ).execute()
...
def upsert_company_vector(
    ...
    _tbl(client, "company_vectors").upsert(
        ...
        on_conflict="ticker,vector_date",
    ).execute()
            ...
            results.append(
                {
                    "ticker": row["ticker"],
                    "vector_date": (
                        datetime.strptime(str(vdate), "%Y-%m-%d").date()
