"""
scheduler.py — Single-cron tick scheduler for the LLM screening agent.
...
def run_tick(max_concurrent: int | None = None) -> dict:
    """One tick of the user screening scheduler. Called every minute."""
    ...
    queued = _queue_due_screenings(client, user_screenings, now_utc, schema)
    launched = _dispatch_due(client, available, now_utc, schema)

    log.info(
        "Agent tick done: queued=%d launched=%d running_before=%d",
        queued,
        launched,
        running_count,
    )
