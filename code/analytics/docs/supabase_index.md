# swingtrader schema index (generated 2026-08-25)

## Relationships & graph
- `ticker_relationship_edge_evidence` (table, ~85,590 @2026-08-25) Ticker relationship edge traceability Goal: - Provide deterministic traceability from ticker_relationship_edge
- `ticker_relationship_edges` (table, ~38,789 @2026-08-25) Ticker Relationship Network (graph-ready adjacency structure) Why: - Avoid scanning/parsing JSONB relationship
...
- `ticker_pair_stats` (table, ~795 @2026-06-15) Ticker Pair Stats (cointegration / pairs-trading metrics on the graph) Why: - The news-derived relationship gr
...
- `news_impact_heads` (table, ~2,692,533 @2026-08-25) news_impact_heads: per-cluster LLM scoring results
- `news_article_embeddings` (table, ~1,827,954) Embedding setup for semantic retrieval over scored news.
- `news_article_tickers` (table, ~727,992) news_article_tickers: ticker mentions extracted from articles
- `news_article_embedding_jobs` (table, ~220,126 @2026-08-25) Embedding setup for semantic retrieval over scored news.
- `news_articles` (table, ~216,666 @2026-08-25) news_articles: article content and metadata
- `news_impact_vectors` (table, ~209,330 @2026-08-25) news_impact_vectors: aggregated impact dimension vectors
- `news_source_dry_days` (table, ~684) Track calendar days where a news source stream has been fully exhausted (all available articles fetched/proces
...
- `news_briefing_subscriptions` (table, ~34 @2026-08-24) News briefing subscriptions: the free, no-account email service that sends a nicely structured PDF of the last
...
## News: topics & claims
- `topic_claim_stats` (table, ~1,394 @2026-08-25) topic_claim_stats — the materialized half
...
- `ticker_sentiment_heads` (table, ~353,761 @2026-08-25) Ticker Sentiment Materialization (pre-exploded, indexed) Why: - swingtrader.ticker_sentiment_heads_v explodes 
- `ticker_coverage_daily` (table, ~63,572 @2026-08-25) Materialize the /quote directory's daily rollup
...
## Screening & scans
- `market_screening_result_rows` (table, ~111,610 @2026-08-25) 
- `market_screening_results` (table, ~3,832 @2026-08-25) 
...
- `market_screenings` (table, ~10 @2026-08-25) 
- `market_screening_subscriptions` (table, ~0) 

## Users, plans & billing
- `user_scan_rows` (table, ~68,397 @2026-08-24) 
- `user_scan_row_notes` (table, ~21,384 @2026-08-24) 
- `user_ticker_chart_workspace` (table, ~4,643 @2026-08-24) Per-user chart workspace: annotations + Chart AI conversation, keyed by ticker
- `user_screening_results` (table, ~2,238 @2026-08-25) ── user_screening_results ──────────────────────────────────────────────────
- `user_scan_jobs` (table, ~237 @2026-08-24) 
- `user_scan_runs` (table, ~216 @2026-08-24) 
- `user_trades` (table, ~36 @2026-08-15) user_trades: per-user trade ledger (buy/sell × long/short) Semantics: side            : 'buy' | 'sell' (execut
- `user_profiles` (table, ~15 @2026-08-18) user_profiles Per-user app state that doesn't belong in auth.users.user_metadata
...
- `user_scheduled_screenings` (table, ~5 @2026-08-25) ── user_scheduled_screenings ────────────────────────────────────────────────
...
- `user_subscriptions` (table, ~0) user_subscriptions Tracks Stripe subscriptions per user
...
## Agents & jobs
- `job_runs` (table, ~256,091 @2026-08-25) 
- `job_health` (table, ~15) 
...
- `research_priced_in_universe` (table, ~5,807 @2026-08-25) 1) The working universe and its schedule.
...
- `research_priced_in` (table, ~34 @2026-08-25) 1) What a price already contains, reconstructed at a point in time.
...
- `search_news_embeddings(query_embedding double precision[], match_count integer, lookback_hours integer, stream_filter text, ticker_filter text[])`
