...
from src.db import get_supabase_client, get_schema, ensure_schema
...
def _check_existing(client: Client, article_hash: str) -> Optional[tuple[int, dict]]:
    ...
    if vec_res.data:
        try:
            impact = json.loads(vec_res.data[0]["impact_json"])
        ...
        head_rows = [
            {
                ...
                "scores_json": json.dumps(head.scores),
                "reasoning_json": json.dumps(head.reasoning),
